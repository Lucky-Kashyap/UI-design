import React from 'react';
import { motion, useReducedMotion } from 'framer-motion';
import { Phone, Mail } from 'lucide-react';
import { SITE } from '@/data/traditionalGroup';

const CTASection = () => {
  const reduce = useReducedMotion();

  return (
    <section
      id="contact"
      className="relative overflow-hidden text-white"
      aria-labelledby="cta-heading"
    >
      <div className="absolute inset-0">
        <img
          src="/media/cta-evening-resort.png"
          alt=""
          aria-hidden="true"
          loading="lazy"
          decoding="async"
          className={`h-full w-full object-cover ${reduce ? '' : 'animate-ken-burns tg-img-drift'}`}
        />
        {/* Animated gif-like layers */}
        {!reduce && (
          <>
            <div className="absolute inset-0 tg-scanline opacity-50" aria-hidden="true" />
            <div className="absolute inset-0 tg-motion-orbs" aria-hidden="true" />
            <div className="absolute inset-0 tg-cta-shimmer" aria-hidden="true" />
          </>
        )}
        <div className="absolute inset-0 bg-tg-deep/78" />
        <div className="absolute inset-0 bg-gradient-to-r from-tg-deep via-tg-navy/75 to-tg-deep/55" />
      </div>
      <div className="absolute inset-x-0 top-0 h-1 tg-prism-line animate-prism-shift bg-[length:200%_100%]" aria-hidden="true" />

      <div className="tg-container relative py-16 md:py-20 lg:py-24">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-14 items-center">
          <motion.div
            initial={reduce ? false : { opacity: 0, x: -20 }}
            whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.5 }}
          >
            <p className="text-label-md uppercase tracking-[0.16em] text-white/60 mb-3">
              Get in touch
            </p>
            <h2 id="cta-heading" className="font-display text-headline-xl text-white mb-4">
              Get a Free Quote
            </h2>
            <div className="mb-5 h-1 w-16 rounded-full tg-prism-line" aria-hidden="true" />
            <p className="text-body-md text-white/75 mb-8 max-w-lg">
              Reach Traditional Group for manufacturing, hospitality, education, or eco-adventure
              enquiries. We are ready to help you take the next step.
            </p>

            <div className="flex flex-col sm:flex-row gap-3">
              <a
                href={SITE.phoneHref}
                className="tg-btn-primary tg-btn-glow bg-white text-tg-navy hover:bg-white/95 gap-2"
              >
                <Phone className="h-4 w-4" />
                {SITE.phoneDisplay}
              </a>
              <a href={`mailto:${SITE.email}`} className="tg-btn-secondary gap-2 hover:scale-[1.02]">
                <Mail className="h-4 w-4" />
                Email us
              </a>
            </div>
          </motion.div>

          <motion.div
            className="rounded-2xl border border-white/15 bg-white/10 backdrop-blur-md p-6 md:p-7 shadow-xl"
            initial={reduce ? false : { opacity: 0, x: 20 }}
            whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.5, delay: 0.08 }}
          >
            <p className="text-[11px] uppercase tracking-[0.14em] text-white/55 mb-3">Contact details</p>
            <address className="not-italic text-sm text-white/80 leading-relaxed space-y-3">
              <p>{SITE.address}</p>
              <p>
                Office:{' '}
                <a href={SITE.officePhoneHref} className="text-white hover:underline">
                  {SITE.officePhoneDisplay}
                </a>
              </p>
              <p>
                Quote line:{' '}
                <a href={SITE.phoneHref} className="text-white hover:underline">
                  {SITE.phoneDisplay}
                </a>
              </p>
              <p>
                Email:{' '}
                <a href={`mailto:${SITE.email}`} className="text-white hover:underline break-all">
                  {SITE.email}
                </a>
              </p>
            </address>
          </motion.div>
        </div>
      </div>
    </section>
  );
};

export default CTASection;
