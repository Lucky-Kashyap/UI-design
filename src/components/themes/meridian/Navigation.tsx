...
const MeridianNavigation = () => {
  ...
  const { scrolled, open, setOpen, active, setActiveHref, handleNavClick } = useThemeNav({
    ...
    >
      <MeridianContainer className="flex min-w-0 items-center justify-between gap-3 lg:gap-6">
        ...
        <button
          type="button"
          className={cn(
            'inline-flex h-11 w-11 min-h-[44px] min-w-[44px] items-center justify-center rounded-xl border transition-colors lg:hidden',
            navSolid
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            <MeridianContainer className="flex flex-col gap-1 py-4">
              {MERIDIAN_NAV_LINKS.map((link, index) => (
                <motion.a
                  key={link.href}
                  href={link.href}
