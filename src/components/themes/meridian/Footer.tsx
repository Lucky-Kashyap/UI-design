...
const MeridianFooter = () => {
  ...
  return (
    <footer className="border-t border-tg-line bg-tg-deep py-8 text-white/70" role="contentinfo">
      <MeridianContainer className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
        <div className="flex min-w-0 flex-col gap-4 sm:flex-row sm:items-center">
          ...
          <MeridianBody className="max-w-xs text-white/70">
            Meridian Atelier preview — {SITE.name} multi-venture collective, Jaipur.
          </MeridianBody>
        </div>
        <div className="flex flex-wrap items-center gap-4">
          <a href={SITE.facebook} target="_blank" rel="noopener noreferrer" className="md-body hover:text-white">
            Facebook
          </a>
          <a href={`mailto:${SITE.email}`} className="md-body hover:text-white">
            {SITE.email}
          </a>
          <span className="md-caption text-white/40">© {year} {SITE.name}</span>
        </div>
      </MeridianContainer>
    </footer>
  );
